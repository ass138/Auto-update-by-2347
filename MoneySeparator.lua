script_name('MoneySeparator.lua')
script_version("0.0.1")
script_authors('YarikVL and regulars by Royan_Millans')
...
if not doesDirectoryExist('moonloader/config') then 
	createDirectory('moonloader/config')--создание папки config
end 
	...
	while true do wait(0)
        ...
        if result then
            if button == 1 then
				...
				if list == 6 then
					os.execute('explorer "https://www.blast.hk/threads/134214/"') -- откроет тему с этим скриптом на BlastHack
				end
                Dialog()
            end
        end
...
function sampev.onShowTextDraw(id, data)
    if ini.main.activeTextdraws then
		...
		--	end
		--if data.text:find("TRADE") or data.text:find("ТОРГ") then
		--	forComma = true
		if tonumber(data.text) then
			data.text = comma_value(data.text)
		end
